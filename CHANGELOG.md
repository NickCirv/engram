...
- **Recall-coverage benchmark (`npm run bench:recall`, ADR-0009).** The honesty
  ...
  baseline. First result (engram on engram, default run): recall@10 33.0% —
  decomposed honestly: candidate generation (callers∪callees) reaches 43.0% of
  co-changed files, PageRank ranking captures 76.6% of that (vs 29.8% random-order
  within the same candidates — the ranker's real +3.2pp lift; 10.4% blind chance).
  All caveats (structural-not-resolve-rate, co-change≠structural, 9.1% no-symbol
  exclusion, self-repo drift) in the output. The live resolve-rate A/B stays budget-gated.
